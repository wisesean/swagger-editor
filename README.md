...
**🕰️ Looking for the older version of API编辑器?** Refer to the [*2.x* branch](https://github.com/swagger-api/swagger-editor/tree/2.x).

API编辑器 lets you edit [Swagger API specifications](https://github.com/swagger-api/swagger-spec/blob/master/versions/2.0.md) in YAML inside your browser and to preview documentations in real time.
...
* [swagger-editor-dist](https://www.npmjs.com/package/swagger-editor-dist) is a dependency-free module that includes everything you need to serve API编辑器 in a server-side project, or a web project that can't resolve npm module dependencies.
...
`deps-check` | Generate a size and licensing report on API编辑器s's dependencies.
...
If you'd like to make code changes to API编辑器, you can start up a Webpack hot-reloading dev server via `npm run dev`.

##### Browser support

API编辑器 works in the latest versions of Chrome, Safari, Firefox, and Edge.
...
This will run API编辑器 (in detached mode) on port 80 on your machine, so you can open it by navigating to `http://localhost` in your browser.  
