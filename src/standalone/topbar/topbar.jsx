        ...
        <div className="topbar">
          <div className="topbar-wrapper">
            ...
            <DropdownMenu {...makeMenuOptions("文件")}>
              <li><button type="button" onClick={this.importFromURL}>从URL导入</button></li>
              <ImportFileMenuItem onDocumentLoad={content => this.props.specActions.updateSpec(content)} />
              <li role="separator"></li>
              {isJson ? [
                  <li key="1"><button type="button" onClick={this.saveAsJson}>保存为JSON</button></li>,
                  <li key="2"><button type="button" onClick={this.saveAsYaml}>转换并且保存为YAML</button></li>
              ] : [
                  <li key="1"><button type="button" onClick={this.saveAsYaml}>保存为YAML</button></li>,
                  <li key="2"><button type="button" onClick={this.saveAsJson}>转换并且保存为JSON</button></li>
              ]}
              <li role="separator"></li>
              <li><button type="button" onClick={this.clearEditor}>清空编辑器</button></li>
            </DropdownMenu>
            <DropdownMenu {...makeMenuOptions("编辑")}>
              <li><button type="button" onClick={this.convertToYaml}>转换为YAML</button></li>
            ...
            { showServersMenu ? <DropdownMenu className="long" {...makeMenuOptions("服务端代码生成")}>
              { this.state.servers
                  .map((serv, i) => <li key={i}><button type="button" onClick={() => this.downloadGeneratedFile("server", serv)}>{serv}</button></li>) }
            </DropdownMenu> : null }
            { showClientsMenu ? <DropdownMenu className="long" {...makeMenuOptions("客户端代码生成")}>
              { this.state.clients
