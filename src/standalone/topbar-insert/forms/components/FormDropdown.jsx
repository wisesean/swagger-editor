...
class FormDropdown extends Component {
  ...
  onChangeWrapper = (event) => {
    if (event.target.value === "请选择" || event.target.value === this.props.placeholderText) {
      const updated = event
  ...
  render() { 
    ...
    const addButton = <a role="button" className="d-inline-block float-right" onClick={this.showAddField} onKeyDown={this.onEnterKeyPress} tabIndex={0}>添加</a>
    ...
    return (
      <div>
        {!this.state.showAddOption &&
          <select 
            value={this.props.selected || this.props.placeholderText || "请选择"} 
            onChange={this.onChangeWrapper}
            className={classNames("custom-select", {"border-danger": !this.props.isValid})} 
          >
            <option value={this.props.placeholderText || "请选择"}>
              {this.props.placeholderText || "请选择"}
            </option>
